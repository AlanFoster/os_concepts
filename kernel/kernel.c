...
void init_serial() {
  ...
  port_byte_out(DATA_REGISTER(port), 0x00);
  ...
  port_byte_out(MODEM_CONTROL_REGISTER(port), 0x03);
  // Enable FIFO
...
int is_transmit_empty() {
  return port_word_in(LINE_STATUS_REGISTER(port)) & 1;
}

// Write to serial, note that this happens least significant bit first.
// Therefore, a character such as ASCII 'O':
// 01101111
// Wil be sent across the wire as:
// 11110110
//
